...
method BUILD {
    ...
    debug 'Adding text from word doc to data hash...';
...
method get_table_data ( :$table_number ) {
    ...
    debug "Removing headers from table data for table $table_number";

    $self->cgp_hash->{'tables'}->{ $table_number }->{'text'} 
        = $self->_get_text_without_headers( table_hash => $table_reference );

    debug "Structuring data for table $table_number";
...
method _structure_data( :$table_hash, :$table_number ) {
    ...
    if ( ( @elements % $total_col_count ) != 0 ) {
        error "Incorrect number of elements in Table $table_number";
        exit 1;
    }

    my $row_count = @elements / $total_col_count;

    info "Reading $row_count row(s) for table $table_number...";
...
method _get_value( :$data, :$lookup ) {
    ...
    if ( $1 ) {
        $value = $1;
        $value =~ s/\s*$//;
        debug "Returning *$value* for $lookup";
    }
    else {
        $value = '';
        warn "Value for $lookup not found";
    }
